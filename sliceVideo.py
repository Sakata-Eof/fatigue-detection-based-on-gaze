    ...
    for person in persons:
        ...
        while cap.isOpened():
            ...
            if len(faces) == 0:
                ...
                print("No face found")
                continue
            top, right, bottom, left = max(faces, key=lambda f: (f[2]-f[0])*(f[1]-f[3]))  # 选取面积最大的面部
            fx = left
            fy = top
            fw = right-left
            fh = bottom-top
            ...
            outfile.write(" ".join([str(frameCount), str(fx), str(fy), str(fw), str(fh), str(lex), str(ley), str(lew), str(leh), str(rex), str(rey), str(rew), str(reh), str(w), str(h)])+"\n")
            frameCount += 1
