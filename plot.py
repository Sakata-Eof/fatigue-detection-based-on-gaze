...
for i in range(4):
    ...
    for j, v in enumerate(scores):
        plt.text(j, v + 0.01, f'{v:.3f}', ha='center', va='bottom', fontsize=18, fontproperties=zhfont1)
    plt.tight_layout()
...
for model in models:
    ...
    pd_mi = pd.DataFrame(model_dict.items(), columns=['Measure', 'Accuracy']).sort_values(by='Accuracy', ascending=False)
    ...
    plt.ylabel('准确率', fontsize=18, fontproperties=zhfont1)
    for i, v in enumerate(scores):
        plt.text(i, v + 0.01, f'{v:.3f}', ha='center', va='bottom', fontsize=18, fontproperties=zhfont1)
    plt.tight_layout()
    plt.xticks(fontsize=18)
