...
def train_cnn(
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X_seq, y_seq, test_size=0.5, random_state=42
    )
...
def train_cnn_kfold(
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_seq)):
        ...
        for epoch in range(100):
            ...
            # 记录日志
            logging.info(
                f"Epoch [{epoch + 1}/100] in fold [{fold + 1}/{n_splits}], "
                f"Train Loss: {train_loss:.4f}, Train Accuracy: {train_acc:.4f}, "
                f"Val Loss: {val_loss:.4f}, Val Accuracy: {val_acc:.4f}"
            )
            print(
            ...
            if val_loss < best_val_loss:
                ...
                if patience_counter >= patience:
                    ...
                    logging.info(
                        f"Early stopping at epoch {epoch + 1} in fold {fold + 1}"
                    )
                    break
            scheduler.step()
...
def PreProcess(X, y, measure=0, standardize=True):
    ...
    for video in videos:
        ...
        if measure == 0:
            ...
            if standardize:
                scaler = MinMaxScaler()
                features[:, 1:] = scaler.fit_transform(
                    features[:, 1:]
                )  # 除帧数以外的数据进行归一化
            ...
            video_df = video_df.dropna(
                subset=["frame_diff", "gaze_x_diff", "gaze_y_diff"]
            )
            ...
            video_df["frame_diff"] = video_df["frame"].diff().fillna(0)
            video_df["velocity_x"] = (
                video_df["gaze_x"].diff().fillna(0) / video_df["frame_diff"]
            ).fillna(0)
            video_df["velocity_y"] = (
                video_df["gaze_y"].diff().fillna(0) / video_df["frame_diff"]
            ).fillna(0)
            video_df["acceleration_x"] = (
                video_df["velocity_x"].diff().fillna(0) / video_df["frame_diff"]
            ).fillna(0)
            video_df["acceleration_y"] = (
                video_df["velocity_y"].diff().fillna(0) / video_df["frame_diff"]
            ).fillna(0)
            features = video_df[
                [
                    "frame",
...
if __name__ == "__main__":
    ...
    for i in range(4):  # 4种数据预处理方式
        X, y, video_boundaries = PreProcess(
            dataF + dataNF, label, measure=i, standardize=True
        )  # 预处理， measure取0-3，
        ...
        for model_name, model in models.items():  # 对比不同模型
            ...
            grid_search = GridSearchCV(
                model, param_grids[model_name], cv=5, scoring="f1", verbose=2
            )  # 超参数调优，verbose=2 显示进度

            grid_search.fit(X_train, y_train)

            logging.info(
                f"Best parameters for {model_name} in measure {i + 1}: {grid_search.best_params_}"
            )
            logging.info(
                f"Best cross-validation score for {model_name} in measure {i + 1}: {grid_search.best_score_}"
            )
            ...
            logging.info(
                f"Classification Report for {model_name} in measure {i + 1}:\n{classification_report(y_test, y_pred)}"
            )
            print(
                f"Classification Report for {model_name} in measure {i + 1}:\n{classification_report(y_test, y_pred)}"
            )
        ...
        for model_name, model in best_models.items():
            ...
            logging.info(
                f"{model_name} Accuracy in measure {i + 1}: {accuracy_score(y_test, y_pred)}"
            )
            print(
                f"{model_name} Accuracy in measure {i + 1}: {accuracy_score(y_test, y_pred)}"
            )
